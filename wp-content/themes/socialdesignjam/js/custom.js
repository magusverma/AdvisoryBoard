...
$(document).ready(function () {
	...
	var windowSize = $(window).width();
		if (windowSize < 480) {
			...
			$('#speakerscarousel').removeClass('item-4');
			$('#speakerscarousel').addClass('item-2');
			$(".speakers-carousel").carousel({
				dispItems: 2,
				direction: "horizontal",
				pagination: false,
	...
	// Countdown //
	$('#countdown').countdown({
		until: new Date(2014, 08 - 1, 0), // new Date(year, mth - 1, day, hr, min, sec) - date/time to count down to 
	...
	// Google Map //
	$('#map_canvas').gmap({
		'center': new google.maps.LatLng(28.54596, 77.272666), // Change this to your desired latitude and longitude
		'zoom': 15,
	...
	var image = {
		url: 'http://karmany.org/socialdesignjam/wp-content/uploads/2014/07/marker.png', // Define the map marker file here
		...
		anchor: new google.maps.Point(17, 63)
	};
	$('#map_canvas').gmap().bind('init', function () {
		$('#map_canvas').gmap('addMarker', {
			'id': 'marker-1',
			'position': '28.54596, 77.272666',
			'bounds': false,
			'icon': image
		}).click(function () {
			$('#map_canvas').gmap('openInfoWindow', {
				'content': '<h4>Indraprastha Institute of Information Technology</h4><p>Okhla, Phase III, Near Govind Puri Metro Station,<br> New Delhi, Delhi - 110020</p>'
			}, this);
		});
	});
